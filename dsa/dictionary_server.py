...
Using dictionary lookup for O(1) performance
"""
...
class TransactionModel:
    ...
    def _ensure_file_exists(self):
        ...
        if not os.path.exists(self.json_file):
            with open(self.json_file, 'w', encoding='utf-8') as f:
                json.dump({}, f)  # Dictionary instead of list
    ...
    def _read_file(self):
        ...
        try:
            with open(self.json_file, 'r', encoding='utf-8') as f:
                data = json.load(f)
                # Convert string keys to integers for proper ID handling
    ...
    def _write_file(self, transactions):
        ...
        transactions_str_keys = {str(k): v for k, v in transactions.items()}
        with open(self.json_file, 'w', encoding='utf-8') as f:
            json.dump(transactions_str_keys, f, indent=2, ensure_ascii=False)
    
    def get_all(self):
        """Get all transactions from file"""
        return self._read_file()
    
    def get_by_key(self, transaction_id):
        """Get single transaction by ID from file - O(1) lookup"""
    ...
    def create(self, data):
        ...
        # Create new transaction (no 'id' field inside)
        new_transaction = {
        ...
        return {'id': new_id, **new_transaction}
    ...
    def update(self, transaction_id, data):
        ...
        if transaction_id in transactions:
            ...
            for key, value in data.items():
                if key != 'id':
                    transactions[transaction_id][key] = value
            
            self._write_file(transactions)
            return {'id': transaction_id, **transactions[transaction_id]}
    ...
    def delete(self, transaction_id):
        ...
        if transaction_id in transactions:
            deleted = transactions.pop(transaction_id)
            self._write_file(transactions)
            return {'id': transaction_id, **deleted}
...
class TransactionController:
    ...
    def get_all_transactions(self):
        ...
        transactions_list = [{'id': k, **v} for k, v in transactions.items()]
        return {
    ...
    def get_transaction(self, transaction_id):
        ...
        try:
            ...
            transaction = self.model.get_by_key(tid)
            if transaction:
                return {'id': tid, **transaction}
            return None
        except ValueError:
            return None
    ...
    def delete_transaction(self, transaction_id):
        ...
        try:
            ...
            if deleted:
                return {"message": f"Transaction {tid} deleted successfully", "deleted": deleted}
            else:
                return {"error": "Transaction not found"}
        except ValueError:
            return {"error": "Invalid transaction ID"}
    ...
    def do_GET(self):
        ...
        if route == "transactions":
            ...
            self._send_json({
                ...
                "performance": "O(1) transaction lookup"
            }, 200)
        else:
...
def run(server_class=HTTPServer, handler_class=MoWiseHandler, port=8000):
    ...
    print("=" * 60)
    print("MoWise - MoMo SMS Processing API v2.0")
    print("Dictionary Lookup - O(1) Performance")
    print("=" * 60)
    ...
    print(f"  GET    /transactions/{{id}}  - Get one transaction (O(1))")
    print(f"  POST   /transactions       - Create new transaction")
    print(f"  PUT    /transactions/{{id}}  - Update transaction (O(1))")
    print(f"  DELETE /transactions/{{id}}  - Delete transaction (O(1))")
